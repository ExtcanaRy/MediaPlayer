...
char xr_operator_dynamic_array(xr_dynamic_array_info *in_info, void **in_value, unsigned long long in_pos, char in_operation)
{
    ...
    case XR_ARRAY_ADD:
        if (in_pos <= in_info->curr_arr_size) {
            in_info->start_addr = realloc(in_info->start_addr, (in_info->curr_arr_size + 1) * in_info->singel_value_size);
            if (!in_info->start_addr) return -1;
            if (in_pos < in_info->curr_arr_size++) {
                char *srcMem, *dstMem;
                for (unsigned long long posCount = in_info->curr_arr_size; posCount > in_pos; posCount--) {
    ...
    case XR_ARRAY_DEL:
        if(in_pos < in_info->curr_arr_size) {
            if (in_pos < in_info->curr_arr_size - 1) {
                char *srcMem, *dstMem;
                for (unsigned long long posCount = in_pos; posCount < in_info->curr_arr_size; posCount++) {
                    srcMem = (char *)in_info->start_addr + (posCount + 1) * in_info->singel_value_size;
                    dstMem = (char *)in_info->start_addr + posCount * in_info->singel_value_size;
                    for (size_t memPos = 0; memPos < in_info->singel_value_size; memPos++) {
                        *dstMem++ = *srcMem++;
            ...
            if(in_info->curr_arr_size-- > 1) in_info->start_addr = realloc(in_info->start_addr, in_info->curr_arr_size * in_info->singel_value_size);
            if(!in_info->start_addr) return -1;
        } else {
            return -1;
        }
